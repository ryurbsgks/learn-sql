...
*/
const PART5_2_5 = `SELECT content.title FROM content LEFT JOIN user ON content.userId=user.id WHERE user.name="jiSungPark"`;

/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-6. JiSungPark이 작성한 content의 category name을 찾기위한 SQL을 작성해주세요.
*/
const PART5_2_6 = `SELECT category.name FROM category LEFT JOIN content_category ON category.id=content_category.categoryId LEFT JOIN content ON content_category.contentId=content.id LEFT JOIN user ON content.userId=user.id WHERE user.name="jiSungPark"`;
...
*/
const PART5_2_7 = `SELECT content.title, content.body, content.created_at FROM content LEFT JOIN content_category ON content.id=content_category.contentId LEFT JOIN category ON content_category.categoryId=category.id WHERE category.name="soccer"`;

/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-8. category의 name이 soccer인 content의 title, body, created_at, user의 name을 찾기위한 SQL을 작성해주세요.
*/
const PART5_2_8 = `SELECT content.title, content.body, content.created_at, user.name FROM user LEFT JOIN content ON user.id=content.userId LEFT JOIN content_category ON content.id=content_category.contentId LEFT JOIN category ON content_category.categoryId=category.id WHERE category.name="soccer"`;

/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-9. duRiCha가 작성한 글의 개수 (컬럼명: ContentCount)를 출력하기 위한 SQL을 작성해주세요.
*/
const PART5_2_9 = `SELECT COUNT(content.userId) AS ContentCount FROM content LEFT JOIN user ON content.userId=user.id WHERE user.name="duRiCha"`;

/*
----------------------------------------------------------------------------------------------
    TODO: Q 5-2-10. 각 user(컬럼명: name)가 작성한 글의 개수 (컬럼명: ContentCount)를 출력하기 위한 SQL을 작성해주세요.
*/
const PART5_2_10 = `SELECT COUNT(content.userId) AS ContentCount, user.name AS name FROM user LEFT JOIN content ON user.id=content.userId GROUP BY user.name`;
